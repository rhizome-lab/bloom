...
pub fn compile_net(op: &str, args: &[SExpr], prefix: &str) -> Result<Option<String>, CompileError> {
    let result = match op {
        "net.get" => {
            if args.len() != 3 {
                return Err(CompileError::InvalidArguments(
                    format!("net.get expects 3 arguments (capability, url, headers), got {}", args.len())
                ));
            }
        ...
        "net.post" => {
            if args.len() != 4 {
                return Err(CompileError::InvalidArguments(
                    format!("net.post expects 4 arguments (capability, url, headers, body), got {}", args.len())
                ));
            }
            let cap = compile_value(&args[0], false)?;
            let url = compile_value(&args[1], false)?;
            ...
            format!("{}__viwo_net_post({}, {}, {}, {})", prefix, cap, url, headers, body)
        }
