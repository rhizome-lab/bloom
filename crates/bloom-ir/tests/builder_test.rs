...
fn test_builder_type_safety() {
    ...
    let _num_expr: SExpr<bloom_ir::Num> = math_sub(SExpr::number(10), SExpr::number(2));

    // str_length returns a number (length)
    let _len_expr: SExpr<bloom_ir::Num> = str_length(SExpr::string("test"));

    let _bool_expr: SExpr<bloom_ir::Bool> = bool_not(SExpr::bool(true));
}
