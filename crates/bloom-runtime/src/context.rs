...
use bloom_core::{Entity, EntityId, Scheduler, WorldStorage};
use bloom_ir::SExpr;
use bloom_runtime_luajit::Runtime as LuaRuntime;
...
impl ExecutionContext {
    ...
    pub fn execute(&self, expr: &SExpr) -> Result<serde_json::Value, crate::ExecutionError> {
        ...
        let lua_code = bloom_runtime_luajit::compile(expr)?;
        ...
        if this_after_json != flattened_this {
            ...
            if let serde_json::Value::Object(after_map) = &this_after_json {
                ...
                if !updates.is_empty() {
                    ...
                    .map_err(|e| {
                        crate::ExecutionError::Runtime(bloom_runtime_luajit::ExecutionError::Lua(
    ...
    fn inject_opcodes(&self, runtime: &LuaRuntime) -> Result<(), crate::ExecutionError> {
        ...
        let sqlite_query_fn = lua.create_function(
            move |lua_ctx,
                ...
                let results = bloom_plugin_sqlite::sqlite_query(
                    &cap_json,
                    this_id,
                    &db_path,
                    &query,
        ...
        let sqlite_execute_fn = lua.create_function(
            move |lua_ctx,
                ...
                let rows_affected = bloom_plugin_sqlite::sqlite_execute(
                    &cap_json,
                    this_id,
                    &db_path,
                    &query,
        ...
        let net_get_fn = lua.create_function(
            move |lua_ctx, (capability, url, headers): (mlua::Value, String, mlua::Value)| {
                ...
                let result = tokio::runtime::Handle::try_current()
                    ...
                    .block_on(bloom_plugin_net::net_get(
                        &cap_json,
                        this_id,
                        &url,
                        headers_map,
            ...
            lua.create_function(
                move |lua_ctx,
                    ...
                    let result = tokio::runtime::Handle::try_current()
                        ...
                        .block_on(bloom_plugin_net::net_post(
                            &cap_json,
                            this_id,
                            &url,
                            headers_map,
        ...
        let vector_insert_fn = lua.create_function(
            move |lua_ctx,
                ...
                let id = bloom_plugin_vector::vector_insert(
                    &cap_json,
                    this_id,
                    &db_path,
                    &key,
        ...
        let vector_search_fn = lua.create_function(
            move |lua_ctx,
                ...
                let results = bloom_plugin_vector::vector_search(
                    &cap_json,
                    this_id,
                    &db_path,
                    &embedding_f32,
        ...
        let vector_delete_fn = lua.create_function(
            move |lua_ctx, (capability, db_path, key): (mlua::Value, String, String)| {
                ...
                let rows_affected =
                    bloom_plugin_vector::vector_delete(&cap_json, this_id, &db_path, &key)
                        .map_err(mlua::Error::external)?;
                Ok(rows_affected)
            },
        )?;
        ...
        let ai_generate_text_fn = lua.create_function(
            move |lua_ctx,
                ...
                let result = tokio::runtime::Handle::try_current()
                    ...
                    .block_on(bloom_plugin_ai::ai_generate_text(
                        &cap_json,
                        this_id,
                        &provider,
                        &model,
            ...
            lua.create_function(
                move |lua_ctx,
                    ...
                    let result = tokio::runtime::Handle::try_current()
                        ...
                        .block_on(bloom_plugin_ai::ai_embed(
        ...
        let ai_chat_fn = lua.create_function(
            move |lua_ctx,
                ...
                let result = tokio::runtime::Handle::try_current()
                    ...
                    .block_on(bloom_plugin_ai::ai_chat(
                        &cap_json,
                        this_id,
                        &provider,
                        &model,
