...
fn get_plugin_path(plugin_name: &str) -> std::path::PathBuf {
    ...
    if cfg!(target_os = "macos") {
        path.push(format!("libbloom_plugin_{}.dylib", plugin_name));
    } else if cfg!(target_os = "windows") {
        path.push(format!("bloom_plugin_{}.dll", plugin_name));
    } else {
        path.push(format!("libbloom_plugin_{}.so", plugin_name));
    }
    path
}
...
fn test_procgen_seed() {
    ...
    let runtime = BloomRuntime::new(storage);
