...
use bloom_runtime_luajit::compile;
use bloom_syntax_typescript::transpile;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::Serve {
            ...
            let runtime = Arc::new(bloom_runtime::BloomRuntime::open(&db)?);
        ...
        Commands::Compile { file, stdout } => {
            ...
            let sexpr: bloom_ir::SExpr = serde_json::from_str(&input)?;
        ...
        Commands::Exec { file } => {
            ...
            let sexpr: bloom_ir::SExpr = serde_json::from_str(&input)?;
            let result = bloom_runtime_luajit::execute(&sexpr)?;
