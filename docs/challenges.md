...
**Challenge**: A custom interpreted language (BloomScript) running inside another interpreted language (JavaScript/TypeScript) could be too slow for complex simulations (e.g., pathfinding, economy, combat math).

**Mitigation**: **Just-In-Time (JIT) Compilation**.

- We have implemented a `compiler` (`packages/scripting/src/compiler.ts`) that translates BloomScript AST directly into native JavaScript functions.
